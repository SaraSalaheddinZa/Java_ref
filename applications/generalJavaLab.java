public class generalJavaLab {
    public static void main (String[] args){
        //Declare and initialize variable for First Name
        String firstName = "Sandy";
        //Declare and initialize variable for Last Name
        String lastName = "Jones";
        //Declare and initialize variable for Movie Title
        String movieName = "The Source Code";
        //Declare and initialize variable for Number of tickets
        int numberOfTickets =8;
        //Declare and initialize variable for Price per ticket
        double pricePerTicket = 10.57;
        System.out.println("First Name: " + firstName);
        System.out.println("Last Name: " + lastName);
        System.out.println("Movie Title: " + movieName);
        System.out.println("Number of Tickets: " + numberOfTickets);
        System.out.println("Price per ticket in USD: " + pricePerTicket);
        // Calculate the total price based on number of tickets and price per ticket. Print the output of the total price
        double totalPrice = numberOfTickets * pricePerTicket;
        System.out.println("Total price of 8 tickets: " + totalPrice);
        //Concatenate first name and last name into new variable username with no space in between
        // first and last name and convert them to lower case
        String userName = firstName.toLowerCase() + lastName.toLowerCase();
        System.out.println("Username: " + userName);

        // Convert the movie title to upper case and assign to variable movieName
        String updatedMovieName = movieName.toUpperCase();
        System.out.println("Updated movie name: " + updatedMovieName);
        //Write multiple print statements to match your output as per the Final Expected Output
        System.out.println("Congratulations! You have successfully booked the ticket \n UserName: " +userName + " \n Movie: " + movieName + "\n Number of Tickets: " + numberOfTickets + "\n Price per ticket: " + pricePerTicket + "\n Total price of 8 tickets: " + pricePerTicket + "\n Thank yor choosing us for booking your movie tickets \n Enjoy your movie!!!");

    }
}
